(ns testlab.operation
  "OperationActor -- one technical-testing/analysis operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (TestLab-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Test Integrity Governor
  ...
  One graph run = one technical-testing/analysis operation (intake ->
  ...
  decision to a human operator (the accredited laboratory director).
  The approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:certification/issue` ALWAYS reaches this node when the
  governor is clean -- see `testlab.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [testlab.testlabllm :as testlabllm]
...
(defn build
  ...
  `testlab.store/Store`).
  opts:
    :advisor      -- a `testlab.testlabllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (testlabllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; TestLab-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (testlabllm/-advise advisor store request)]
            {:proposal p :audit [(testlabllm/trace request p)]})))

      ;; Test Integrity Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
